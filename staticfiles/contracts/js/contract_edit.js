document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSuccessPopup() {
        if (successPopupTitle) {
            successPopupTitle.textContent = 'Cập nhật hợp đồng lao động thành công';
        }
        if (successPopup) {
    ...
    function updateMucLuong() {
        ...
        } else if (contractTypeSelect.value === 'PART_TIME') {
            mucLuong = getPartTimeHourlyRate() * (parseFloat(soGioLamToiThieuInput.value) || 0);
        }
    ...
    if (contractTypeSelect) contractTypeSelect.addEventListener('change', applyRules);
    if (luongCoBanInput) luongCoBanInput.addEventListener('input', updateMucLuong);
    if (luongTheoGioSelect) {
        luongTheoGioSelect.addEventListener('change', function () {
            if (luongTheoGioSelect.value === 'khac') showLuongTheoGioKhac();
            else hideLuongTheoGioKhac();
            updateMucLuong();
        });
    }
    if (luongTheoGioKhacInput) luongTheoGioKhacInput.addEventListener('input', updateMucLuong);
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', function () { confirmCancelPopup.style.display = 'flex'; });
    if (confirmNoBtn) confirmNoBtn.addEventListener('click', function () { confirmCancelPopup.style.display = 'none'; });
    if (confirmYesBtn) confirmYesBtn.addEventListener('click', function () { window.location.href = contractForm.dataset.contractListUrl; });
    if (confirmCancelPopup) {
        confirmCancelPopup.addEventListener('click', function (event) {
            if (event.target === confirmCancelPopup) confirmCancelPopup.style.display = 'none';
        });
    }

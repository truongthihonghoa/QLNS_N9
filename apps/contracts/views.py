...
def contract_list_view(request):
    ...
    if role == "Nhân viên":
        # Nhân viên chỉ thấy hợp đồng của chính mình
        try:
        ...
        contracts_list.append(
            {
                ...
                "ngay_sinh": nv.ngay_sinh.strftime("%d/%m/%Y") if nv and nv.ngay_sinh else "",
                "sdt_nv": nv.sdt if nv else "",
                ...
                "trang_thai_code": trang_thai_code,
                "trang_thai_display": trang_thai_display,
            }
        )
...
def contract_detail_view(request, contract_id):
    ...
    # Kiểm tra quyền: Nhân viên chỉ được xem hợp đồng của chính mình
    if role == "Nhân viên":
        try:
            user_ma_nv = request.user.taikhoan.ma_nv
            if hd.ma_nv != user_ma_nv:
                return JsonResponse({"success": False, "message": "Bạn không có quyền xem hợp đồng của nhân viên khác."}, status=403)
        except Exception:
            return JsonResponse({"success": False, "message": "Hệ thống lỗi hoặc chưa gán nhân viên."}, status=403)
